...
namespace MassEffectModBuilder.Merge
{
    ...
    public record class UpdateAsset(string TargetFile, string TargetM3m, string VanillaEntryName, string NewEntryName, string AssetFileName, bool CanMergeAsNew = false) : IMergeTask
    {
        public void RunMergeTask(MergeBuilderContext context)
        {
            ...
            var destinationPath = Path.Combine(context.ModBuilderContext.MergeModsFolder, Path.GetFileName(AssetFileName));
            ...
            context.Builder.AddChange(TargetFile, new AssetUpdate(VanillaEntryName, NewEntryName, Path.GetFileName(AssetFileName), CanMergeAsNew));
        }
    }
}
